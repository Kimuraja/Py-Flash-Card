...
ko_alp = choice(df.values)


def guess():
    canvas.itemconfig(canvas_image, image=front_img)
    canvas.delete("txt")
    canvas.create_text(400, 263, text=f"{ko_alp[0]}", font=("Ariel", 60, "bold"), tags="txt")


def swap_bg():
    canvas.itemconfig(canvas_image, image=back_img)
    canvas.delete("txt")
    canvas.create_text(400, 263, text=f"{ko_alp[1]}", font=("Ariel", 60, "bold"), tags="txt")
...
wrong_button = Button(image=wrong, highlightthickness=0, command=swap_bg)
...
swap = window.after(3000, swap_bg)
